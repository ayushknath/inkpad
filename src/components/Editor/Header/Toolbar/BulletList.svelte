...
<Button
  size="icon"
  variant="outline"
  class={editorStates.editorState.editor?.isActive("bulletList")
    ? "bg-gray-800 text-white outline-transparent hover:bg-gray-600 hover:text-white hover:outline-transparent"
    : ""}
  onclick={() =>
    editorStates.editorState.editor?.chain().focus().toggleBulletList().run()}
  disabled={!editorStates.editorState.editor
    ?.can()
    .chain()
    .focus()
    .toggleBulletList()
